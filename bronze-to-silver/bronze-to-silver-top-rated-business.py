...
# MAGIC %md
# MAGIC #### Top Rated Business
# MAGIC business with:
# MAGIC - 3.5 starts or more
# MAGIC - 50 reviews at least

# COMMAND ----------

# Try to read the Yelp business data from the JSON file using Spark's read method
# If an error occurs while reading the file, print an error message and log the error
try:
    df_yelp_academic_dataset_business = spark.read.json(
        "/mnt/yelp/bronze/yelp_academic_dataset_business.json"
    )
except Exception as e:
    logging.error(f"An error occurred while reading the file: {e}")
    raise

# Select only the columns we consider relevant for our use-case
top_rated_business = df_yelp_academic_dataset_business[
    ["name", "stars", "review_count", "state", "city", "categories", "business_id"]
]

# Filter the Yelp businesses based on stars and review_count criteria
top_rated_business = top_rated_business.filter(col("stars") >= 3.5).filter(
    col("review_count") >= 50
)


# Write the resulting DataFrame to a Delta Lake table in a specified location
# The table is partitioned by state for optimized query performance
# Log a success message to confirm that the data has been written
# If an error occurs during the write process, print an error message and log the error
try:
    top_rated_business.write.format("delta").partitionBy("state").mode(
        "overwrite"
    ).save("/mnt/yelp/silver/top_rated_business")
